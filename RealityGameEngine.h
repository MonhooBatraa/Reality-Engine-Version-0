...
public:
	...
	realitySprite(int w, int h)
	{
		Create(w, h);
	}

	realitySprite(wstring sFile)
	{
		if (!Load(sFile))
			Create(8, 8);
	}
...
public:
	...
	void DrawSprite(int x, int y, realitySprite* sprite)
	{
		if (sprite == nullptr)
			return;
	...
	void DrawPartialSprite(int x, int y, realitySprite* sprite, int ox, int oy, int w, int h)
	{
		if (sprite == nullptr)
			return;
...
public:
	void Start()
	{
		...
		// Star the thread
		thread t = thread(&RealityGameEngine::GameThread, this);
	...
	void GameThread()
	{
		...
		while (m_bAtomActive)
		{
			...
			while (m_bAtomActive)
			{
				...
				wchar_t s[256];
				swprintf_s(s, 256, L"Reality 3D Game Engine - %s - FPS: %3.2f - %d ", m_sAppName.c_str(), 1.0f / fElapsedTime, events);
				SetConsoleTitle(s);
...
atomic<bool> RealityGameEngine::m_bAtomActive = false;
condition_variable RealityGameEngine::m_cvGameFinished;
mutex RealityGameEngine::m_muxGame;